    ...
    <div>
        <div class="commentAndReplies">
            <div class="commentItem">
                ...
                <button class="btn btn-danger commentDeleteBtn" @click="showDeleteModal=true">삭제하기</button>
            </div>
            <div class="replyItems">
                <ul class="replies list-group">
                    <li class="commentDetails list-group-item" v-for="reply in comment.replies" :key="reply">
                        <ReplyListItem :reply="reply" @replyDeleted="this.$emit('commentDeleted');" />
                    </li>
                </ul>
            </div>
        </div>
...
export default {
    ...
    methods: {
        sendDeleteToServer() {
            ...
            axios
                .delete( this.$store.state.backendUrl + '/admin/comments/' + this.comment.comment.id, {
                    headers: {
