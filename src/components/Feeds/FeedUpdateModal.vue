<template>
    <div>
        ...
        <input type="file" id="chooseFile" name="chooseFile" accept="image/*" @change="changedThumbnail" >
        ...
        <textarea class="mb-2 form-control" aria-label="With textarea" style="height: 100px" v-model="content" />

        <Tech :techs="techs" @techChanged=changedTechs($event) />
        
        <label class="input-group-text" for="inputGroupSelect01">Sos</label>
        <select class="mb-2 form-select" id="inputGroupSelect01" v-model="sos">
        ...
        <select class="mb-2 form-select" id="inputGroupSelect01" v-model="step">
        ...
        <div v-if = "checkDeployedUrlReq()">
            <span class="input-group-text" id="basic-addon1">DeployedUrl</span>
            <input type="text" class="mb-2 form-control" v-model="deployedUrl">
        </div>

        <span class="input-group-text" id="basic-addon1">StorageUrl</span>
        ...
        <button class="btn btn-outline-dark" @click="showUpdateModal=this.$emit('updateModalClose')">아니요. 취소합니다</button>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        sendUpdateToServer() {
            ...
            axios.put( this.$store.state.backendUrl + '/admin/feeds/' + this.feedData.id, 
                    updateFeedData,{
                    headers: {
